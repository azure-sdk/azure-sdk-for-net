// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;

namespace Azure.Template.Contoso
{
    /// <summary> Model factory for models. </summary>
    public static partial class TemplateContosoModelFactory
    {
        /// <summary> Initializes a new instance of WidgetSuite. </summary>
        /// <param name="name"> The widget name. </param>
        /// <param name="manufacturerId"> The ID of the widget's manufacturer. </param>
        /// <param name="sharedModel"> The faked shared model. </param>
        /// <returns> A new <see cref="Contoso.WidgetSuite"/> instance for mocking. </returns>
        public static WidgetSuite WidgetSuite(string name = null, string manufacturerId = null, FakedSharedModel sharedModel = null)
        {
            return new WidgetSuite(name, manufacturerId, sharedModel);
        }

        /// <summary> Initializes a new instance of FakedSharedModel. </summary>
        /// <param name="tag"> The tag. </param>
        /// <param name="createdAt"> The created date. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="tag"/> is null. </exception>
        /// <returns> A new <see cref="Contoso.FakedSharedModel"/> instance for mocking. </returns>
        public static FakedSharedModel FakedSharedModel(string tag = null, DateTimeOffset createdAt = default)
        {
            if (tag == null)
            {
                throw new ArgumentNullException(nameof(tag));
            }

            return new FakedSharedModel(tag, createdAt);
        }
    }
}
