...
namespace Azure.Compute.Batch
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<PublicIpAddressConfiguration>)this).Write(writer, new ModelReaderWriterOptions("W"));
        ...
        void IJsonModel<PublicIpAddressConfiguration>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            ...
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(PublicIpAddressConfiguration)} does not support '{format}' format.");
            }

            writer.WriteStartObject();
            if (IpAddressProvisioningType.HasValue)
            {
                writer.WritePropertyName("provision"u8);
                writer.WriteStringValue(IpAddressProvisioningType.Value.ToString());
            }
            if (!(IpAddressIds is ChangeTrackingList<string> collection && collection.IsUndefined))
            {
        ...
        internal static PublicIpAddressConfiguration DeserializePublicIpAddressConfiguration(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            return new PublicIpAddressConfiguration(Optional.ToNullable(provision), ipAddressIds ?? new ChangeTrackingList<string>(), serializedAdditionalRawData);
        }
        ...
        BinaryData IPersistableModel<PublicIpAddressConfiguration>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(PublicIpAddressConfiguration)} does not support '{options.Format}' format.");
            }
        }
