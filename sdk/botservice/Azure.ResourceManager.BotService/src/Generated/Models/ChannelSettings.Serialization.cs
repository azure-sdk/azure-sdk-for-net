...
{
    public partial class ChannelSettings : IUtf8JsonSerializable, IJsonModel<ChannelSettings>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<ChannelSettings>)this).Write(writer, ModelSerializationExtensions.WireOptions);

        void IJsonModel<ChannelSettings>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
        ...
        {
            var format = options.Format == "W" ? ((IPersistableModel<ChannelSettings>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(ChannelSettings)} does not support writing '{format}' format.");
            }
        ...
        ChannelSettings IJsonModel<ChannelSettings>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ChannelSettings>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(ChannelSettings)} does not support reading '{format}' format.");
            }

            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeChannelSettings(document.RootElement, options);
        }

        internal static ChannelSettings DeserializeChannelSettings(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            IReadOnlyList<Site> sites = default;
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("sites"u8))
                {
                    ...
                    {
                        array.Add(Site.DeserializeSite(item, options));
                    }
                    sites = array;
                    continue;
                }
            ...
            return new ChannelSettings(
                extensionKey1,
                extensionKey2,
                sites ?? new ChangeTrackingList<Site>(),
                channelId,
                channelDisplayName,
                botId,
                botIconUrl,
        ...
        BinaryData IPersistableModel<ChannelSettings>.Write(ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ChannelSettings>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(ChannelSettings)} does not support writing '{options.Format}' format.");
            }
        }

        ChannelSettings IPersistableModel<ChannelSettings>.Create(BinaryData data, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ChannelSettings>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(ChannelSettings)} does not support reading '{options.Format}' format.");
            }
        }

        string IPersistableModel<ChannelSettings>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
