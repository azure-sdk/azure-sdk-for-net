...
namespace Azure.ResourceManager.ContainerService.Models
{
    public partial class NodeImageSelection : IUtf8JsonSerializable, IJsonModel<NodeImageSelection>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(SelectionType), out propertyOverride);
            ...
            {
                builder.Append("  type: ");
                builder.AppendLine($"'{SelectionType.ToString()}'");
            }

            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(CustomNodeImageVersions), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("  customNodeImageVersions: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsCollectionDefined(CustomNodeImageVersions))
                {
                    if (CustomNodeImageVersions.Any())
                    {
                        builder.Append("  customNodeImageVersions: ");
                        builder.AppendLine("[");
                        foreach (var item in CustomNodeImageVersions)
                        {
                            BicepSerializationHelpers.AppendChildObject(builder, item, options, 4, true, "  customNodeImageVersions: ");
                        }
                        builder.AppendLine("  ]");
                    }
                }
