...
namespace Azure.ResourceManager.StreamAnalytics
{
    ...
    public partial class StreamingJobResource : ArmResource
    {
        ...
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        public virtual async Task<ArmOperation> StopAsync(WaitUntil waitUntil, StopStreamingJobContent content = null, CancellationToken cancellationToken = default)
        {
            using var scope = _streamingJobClientDiagnostics.CreateScope("StreamingJobResource.Stop");
            scope.Start();
            try
            {
                var response = await _streamingJobRestClient.StopAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, content, cancellationToken).ConfigureAwait(false);
                var operation = new StreamAnalyticsArmOperation(_streamingJobClientDiagnostics, Pipeline, _streamingJobRestClient.CreateStopRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, content).Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionResponseAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        /// <param name="content"> Parameters applicable to a stop streaming job operation. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        public virtual ArmOperation Stop(WaitUntil waitUntil, StopStreamingJobContent content = null, CancellationToken cancellationToken = default)
        {
            using var scope = _streamingJobClientDiagnostics.CreateScope("StreamingJobResource.Stop");
            scope.Start();
            try
            {
                var response = _streamingJobRestClient.Stop(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, content, cancellationToken);
                var operation = new StreamAnalyticsArmOperation(_streamingJobClientDiagnostics, Pipeline, _streamingJobRestClient.CreateStopRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, content).Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletionResponse(cancellationToken);
                return operation;
