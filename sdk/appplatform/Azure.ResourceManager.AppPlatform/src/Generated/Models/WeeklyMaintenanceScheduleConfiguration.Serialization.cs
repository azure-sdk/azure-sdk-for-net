// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.AppPlatform.Models
{
    public partial class WeeklyMaintenanceScheduleConfiguration : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("hour"u8);
            writer.WriteNumberValue(Hour);
            writer.WritePropertyName("day"u8);
            writer.WriteStringValue(Day.ToString());
            writer.WritePropertyName("frequency"u8);
            writer.WriteStringValue(Frequency.ToString());
            writer.WriteEndObject();
        }

        internal static WeeklyMaintenanceScheduleConfiguration DeserializeWeeklyMaintenanceScheduleConfiguration(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            int hour = default;
            Optional<int> durationHours = default;
            WeekDay day = default;
            Frequency frequency = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("hour"u8))
                {
                    hour = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("durationHours"u8))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        continue;
                    }
                    durationHours = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("day"u8))
                {
                    day = new WeekDay(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("frequency"u8))
                {
                    frequency = new Frequency(property.Value.GetString());
                    continue;
                }
            }
            return new WeeklyMaintenanceScheduleConfiguration(frequency, hour, Optional.ToNullable(durationHours), day);
        }
    }
}
