...
namespace Azure.ResourceManager.Grafana
{
    ...
    public partial class ManagedGrafanaResource : ArmResource
    {
        ...
        /// <summary> Gets a collection of GrafanaPrivateLinkResourceDataResources in the ManagedGrafana. </summary>
        /// <returns> An object representing collection of GrafanaPrivateLinkResourceDataResources and their operations over a GrafanaPrivateLinkResourceDataResource. </returns>
        public virtual GrafanaPrivateLinkResourceDataCollection GetAllGrafanaPrivateLinkResourceData()
        {
            return GetCachedClient(client => new GrafanaPrivateLinkResourceDataCollection(client, Id));
        }
        ...
        [ForwardsClientCalls]
        public virtual async Task<Response<GrafanaPrivateLinkResourceDataResource>> GetGrafanaPrivateLinkResourceDataAsync(string privateLinkResourceName, CancellationToken cancellationToken = default)
        {
            return await GetAllGrafanaPrivateLinkResourceData().GetAsync(privateLinkResourceName, cancellationToken).ConfigureAwait(false);
        }
        ...
        /// <exception cref="ArgumentException"> <paramref name="privateLinkResourceName"/> is an empty string, and was expected to be non-empty. </exception>
        [ForwardsClientCalls]
        public virtual Response<GrafanaPrivateLinkResourceDataResource> GetGrafanaPrivateLinkResourceData(string privateLinkResourceName, CancellationToken cancellationToken = default)
        {
            return GetAllGrafanaPrivateLinkResourceData().Get(privateLinkResourceName, cancellationToken);
        }
        ...
        {
            using var scope = _managedGrafanaClientDiagnostics.CreateScope("ManagedGrafanaResource.Refresh");
            scope.Start();
            try
            {
                var response = await _managedGrafanaRestClient.RefreshAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken).ConfigureAwait(false);
                var operation = new GrafanaArmOperation(_managedGrafanaClientDiagnostics, Pipeline, _managedGrafanaRestClient.CreateRefreshRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name).Request, response, OperationFinalStateVia.AzureAsyncOperation);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionResponseAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        public virtual ArmOperation Refresh(WaitUntil waitUntil, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var response = _managedGrafanaRestClient.Refresh(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken);
                var operation = new GrafanaArmOperation(_managedGrafanaClientDiagnostics, Pipeline, _managedGrafanaRestClient.CreateRefreshRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name).Request, response, OperationFinalStateVia.AzureAsyncOperation);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletionResponse(cancellationToken);
                return operation;
